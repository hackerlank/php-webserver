...
SwaggerOperation.prototype.urlify = function(args) {
  ...
  for(var i = 0; i < params.length; i ++){
    ...
    if(param.paramType === 'base64') {
      if (args[param.name] !== undefined) {
          if(param.dataType == 'object'){
            dataObj[param.name] = JSON.parse(args[param.name]);
          } else {
            dataObj[param.name] = args[param.name];
          } 
      	  is_dataObj = true;
      }
    }
  ...
  if(is_dataObj){      
  	  var dataUrl = 'data='+encodeURIComponent(Base64.encodeURI(JSON.stringify(dataObj)));      
  	  if (url.indexOf("?") === -1) {
  	 	 url += '?' + dataUrl;
  	 } else {
  	 	 url += '&' + dataUrl;
